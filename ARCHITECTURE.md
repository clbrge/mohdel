...
One-shot routes (`/v1/embed`, `/v1/image`, `/v1/transcription`) run their exchange on a task of their own once a session is acquired: a handler future dropped mid-exchange leaves the session to finish the call and return to the pool. Embedders that need the two phases apart call `dispatch_embed` / `dispatch_image` / `dispatch_transcription`: dropped before it returns, the request never reaches a session; once it returns, the `Exchange` owns the result and can be awaited from any task. `handle_*` is `dispatch_*` awaited in place.
