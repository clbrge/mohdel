...
Mohdel's wire protocol is language-agnostic. The JS client in `js/client/` is the first implementation, not the only one; a Python / Go / Ruby / Swift / Elixir client is a self-contained contribution.
...
The canonical `js/client/` is ~100 LOC, and a port in most languages lands in a similar range. Name it `mohdel-client-<lang>` (`mohdel-client-python`, `mohdel-client-go`), in the repo or as a satellite.

## Type declarations

The source is JavaScript with JSDoc; `.d.ts` files are generated from it by
`npm run build:types` and emitted beside each module. They are build output —
gitignored, regenerated by `prerelease` so a publish cannot ship stale ones,
and listed in `files` so they reach the tarball.

Emit them anywhere else and the package silently loses its types: the
declarations reference each other through `#core/*`, which resolves to
`./js/core/*.js`, so a consumer looks for `js/core/*.d.ts` and finds nothing —
and `skipLibCheck: true`, which nearly every consumer has on, turns that into
`any` instead of an error. `build:types` deletes the previous output before
running `tsc`, because tsc otherwise resolves those specifiers to the stale
declarations and refuses to overwrite its own input.

Types therefore come from the JSDoc: an annotated `@param` or `@typedef`
produces a real type, and anything unannotated becomes `any`. The wire
contract in `js/core/` is fully annotated and types exactly; the factory's
fluent surface is a Proxy and cannot be inferred, so it stays loose.

## Naming a provider or a creator

**A provider is the service you get an API key from — not the company.** The key
is that service's name, lowercased: `anthropic`, `groq`, `openrouter`. Where a
company sells inference through more than one service, the service wins:
`gemini` is Google's Gemini API, and a Vertex adapter would be `vertex`, not a
second `google`. `qwen` is Qwen Cloud, which is Alibaba's. The picker label in
`src/cli/onboard.js` names that same service — bare where service and company
coincide (`Anthropic`, `Groq`), qualified where they do not (`Google Gemini`,
`Qwen Cloud`, `Xiaomi MiMo`).

**A creator is the organisation that trained the weights**, in
`src/lib/creators.js`: `google`, `alibaba`, `meta`, `bfl`. The label is that
organisation's own styling — `Z AI`, `Moonshot AI`, `xAI`.

The two keys match only when a company serves its own models, which is most of
them. `groq/gpt-oss` is creator `openai`; `cerebras/qwen-3.8-27b` is creator
`alibaba`.

Neither registry records which creators a provider serves. That is a property
of a catalog, not of a provider — a router's line-up changes week to week — and
it is read off the catalog wherever it is needed.

Descriptions in the picker must carry **no model version number**. A generation
named in prose ("Gemini 2.5/3", "Llama 4") is stale the moment the vendor ships
the next one and nothing here updates it; describe capability and commercial
terms instead. A test enforces this.
...
> [PROTOCOL.md](PROTOCOL.md) is the authoritative wire-level specification: envelope shape, event grammar, control messages, state invariants, canonical error types, compliance checklist.
...
Two files, both alphabetical. `_registry.js` is the allowlist `run()` checks
the provider name against before importing an adapter by name, so an adapter
missing from it fails the call with `unknown provider`:

```js
// js/session/adapters/_registry.js
export const ADAPTER_NAMES = Object.freeze([
  'anthropic', 'echo', 'myprovider', 'openai'
])
```
...
A provider selling speed lanes declares them in `SPEED_LANES`, and an image
adapter goes in `IMAGE_ADAPTER_NAMES`. `test/unit/adapter-registry.test.js`
asserts each list matches the files on disk.
...
| `catalog.local.json` | This installation's own catalog fields and tags |
| `providers.json` | Provider-level rate limits |
| `excluded.json` | Models hidden from `list` and `curate` |
| `default.json` | Default model, and the chosen coding agent |
